// provider_mainpage.js (aligned to ongoing/finished, with route fallback & covid cascade)
...
var vueinst = new Vue({
  ...
  data: {
    ...
    // UI state
    mode: "orders", // orders, profile, setting, products, covid_report
  ...
  methods: {
    ...
    viewOrderDetails(order) {
      ...
      details += `Order ID: ${order.service_id}\n`;
      details += `Customer: ${order.customer_name}\n`;
      details += `Customer Email: ${order.customer_email}\n`;
      details += `Customer COVID Status: ${order.customer_covid_status}\n`;
      details += `Order COVID Status: ${order.order_covid_status}\n`;
      details += `Order Time: ${this.formatOrderTime(order.order_timestamp)}\n`;
      details += `Order Status: ${order.order_status}\n`;
      details += `Total Amount: $${order.amount}\n\n`;
      details += `Delivery Address: ${order.dropoff_address}\n`;
      details += `Pickup Address: ${order.pickup_address}\n\n`;
      details += `Items:\n`;
      (order.items || []).forEach((item) => {
        details += `- ${item.product_name}: $${item.price} x ${
          item.quantity
    ...
    // ---------- Covid status ----------
    /** 与 user 端一致：上报后再触发“级联修改相关订单/用户”的后端逻辑 */
    async reportCovid(status) {
      ...
      try {
        ...
        // 2) 再级联修改与我相关的订单/用户的状态（server 侧实现）
        await this.change_relevant_covid();
    ...
    /** 级联：把相关订单 order_covid_status 改为我上报的状态，并把涉及用户标 Yellow（由后端实现） */
    async change_relevant_covid() {
      try {
        const data = await apiGET("/change_relevant_covid");
        console.log(
          "Relevant orders' covid status updated:",
          data && data.message
        );
      } catch (e) {
        console.warn("change_relevant_covid failed:", e && e.status);
      }
    },
