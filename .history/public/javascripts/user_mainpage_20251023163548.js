var vueinst = new Vue({
  ...
  methods: {
    ...
    getProvider() {
      ...
      xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          ...
          ptr.filtered_provider = ptr.provider; // 初始化过滤后的列表
    ...
    Search() {
      ...
      if (searchInput) {
        this.search_query = searchInput.value.toLowerCase().trim();
        this.filterProviders();
      }
    },
    filterProviders() {
      if (!this.search_query) {
        // 如果搜索框为空，显示所有provider
        this.filtered_provider = this.provider;
      } else {
        // 根据搜索关键词过滤provider
        this.filtered_provider = this.provider.filter(function(provider) {
          return provider.name.toLowerCase().includes(this.search_query) ||
                 (provider.description && provider.description.toLowerCase().includes(this.search_query));
        }.bind(this));
      }
    },
